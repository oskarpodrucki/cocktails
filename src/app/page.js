	...
	if (isLoading) {
		return (
			<div className='flex justify-center items-center h-screen'>
				<p className='text-gray-900 animate-pulse'>Loading cocktails...</p>
			</div>
		);
	}
	...
	return (
		<div className='flex flex-row flex-wrap justify-center items-center h-screen p-4 bg-gradient-to-b from-gray-100 to-white'>
			{alcohols.map((alcohol, index) => (
				<Card
					key={index}
					className='w-[300px] h-[700px] m-4 shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300'>
					<CardHeader className='bg-gray-200 rounded-t-lg p-4'>
						<CardTitle className='text-lg font-bold'>
							{alcohol.strDrink}
						</CardTitle>
						<CardDescription className='text-sm'>
							{alcohol.strCategory} - {alcohol.strAlcoholic}
						</CardDescription>
					</CardHeader>
					<ScrollArea className='h-[600px]'>
						<CardContent className='p-4 overflow-y-auto h-full'>
							...
							<ul className='list-disc list-inside'>
								{Object.keys(alcohol)
									.filter(
										(key) => key.startsWith("strIngredient") && alcohol[key]
									)
									.map((ingredient, idx) => (
										<li key={idx}>
